...
export default function Header() {
  ...
  return (
    <header className="flex justify-between items-center w-full px-margin-mobile md:px-margin-desktop h-16 bg-surface border-b border-outline-variant sticky top-0 z-10">
      <div className="flex items-center gap-4">
        <h1 className="font-headline-md text-headline-md font-bold text-primary tracking-tighter md:hidden">AttendSecure</h1>
        <div className="font-code-inline text-code-inline text-primary flex items-center gap-1 px-2">
          <span>{hours}</span><span className="terminal-cursor">:</span><span>{minutes}</span>
        </div>
      </div>
      <div className="flex items-center gap-6">
        <button className="relative p-2 rounded hover:bg-surface-variant transition-colors text-primary flex items-center justify-center group">
